...
export default function EditBon({ bonEntry: bonId, onClose, onSave }) {
  // State management
  ...
  const total = useMemo(() => {
    return formData.produits.reduce((acc, item) => {
      const prix = parseFloat(item.prix_achat || 0);
  ...
  const validateForm = () => {
    ...
    formData.produits.forEach((produit, index) => {
      ...
      if (!produit.prix_achat || isNaN(parseFloat(produit.prix_achat)) || parseFloat(produit.prix_achat) <= 0) {
        errors[`produits.${index}.prix_achat`] = "Veuillez entrer un prix valide";
        isValid = false;
      }
    });
  ...
  useEffect(() => {
    console.log('EditBon component mounted with bonId:', bonId);
    if (bonId) {
      fetchInitialData(bonId);
    } else {
      console.error('Error: No bonId provided to EditBon component');
      setLoadingError('Identifiant du bon d\'entrée manquant.');
      setLoading(false);
    }
  }, [bonId]);
  ...
  const fetchInitialData = async (bonId) => {
    ...
    try {
      // Fetch data sequentially to isolate potential issues
      console.log('Fetching produits...');
      let produitsResponse;
      try {
        produitsResponse = await fetch(`${API_BASE_URL}/produits`);
        ...
        console.error('Erreur lors du chargement des produits:', prodErr);
        throw new Error(`Problème avec l'API produits: ${prodErr.message}`);
      }
      
      console.log('Fetching fournisseurs...');
      let fournisseursResponse;
      try {
        fournisseursResponse = await fetch(`${API_BASE_URL}/fournisseurs`);
        if (!fournisseursResponse.ok) throw new Error(`Erreur fournisseurs: ${fournisseursResponse.status}`);
        const fournisseursData = await fournisseursResponse.json();
        console.log('Fournisseurs data:', fournisseursData);
        if (fournisseursData && fournisseursData.data) {
          setFournisseursList(fournisseursData.data);
        } else {
          throw new Error('Format de réponse fournisseurs invalide');
        }
      } catch (fournErr) {
        console.error('Erreur lors du chargement des fournisseurs:', fournErr);
        throw new Error(`Problème avec l'API fournisseurs: ${fournErr.message}`);
      }
      
      console.log('Fetching bon entrée details...');
      try {
        const bonResponse = await fetch(`${API_BASE_URL}/bonentrees/${bonId}`);
        if (!bonResponse.ok) throw new Error(`Erreur bon entrée: ${bonResponse.status}`);
        const bonData = await bonResponse.json();
        console.log('Bon entrée data:', bonData);
        ...
        if (bonData && bonData.data) {
          ...
          const transformedFormData = {
            ...
            fournisseur_id: bonDetails.fournisseur ? bonDetails.fournisseur.id.toString() : '',
            reference: bonDetails.reference || '',
            date: bonDetails.dateEntree || '',
          ...
          throw new Error('Format de réponse bon entrée invalide');
        }
      } catch (bonErr) {
        console.error('Erreur lors du chargement du bon entrée:', bonErr);
        throw new Error(`Problème avec l'API bon entrée: ${bonErr.message}`);
      }
    } catch (err) {
  ...
  const handleProductChange = (index, field, value) => {
    ...
    // Si le champ modifié est produit_id, mettre à jour automatiquement le prix
    if (field === 'produit_id' && value) {
      ...
      if (selectedProduct) {
        ...
        if (!isNaN(prix)) {
          updatedProduits[index].prix_achat = prix.toString();
        }
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Ensure bonId exists
    if (!bonId) {
      setError('ID du bon d\'entrée manquant.');
      setSubmitting(false);
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/bonentrees/${bonId}`, {
        method: 'PUT',
        headers: {
      ...
      if (responseData) {
        ...
        const formattedBon = {
          ...
          dateEntree: updatedBon.dateEntree || formData.date,
          prixTotal: updatedBon.prixTotal || total,
          status: updatedBon.status || formData.status,
          fournisseur: {
            id: updatedBon.fournisseur?.id || formData.fournisseur_id,
            nom: updatedBon.fournisseur?.nom || 
                fournisseursList.find(f => f.id == formData.fournisseur_id)?.name || 
                'Fournisseur inconnu'
          },
      ...
      setError(err.message || 'Une erreur est survenue lors de la mise à jour du bon d\'entrée');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-full overflow-auto animate-fadeIn">
        <div className="sticky top-0 bg-white z-10 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-medium text-gray-900 flex items-center">
            <ShoppingCart className="mr-2" /> Modifier le bon d'entrée : {formData.reference}
          </h3>
          <button
            onClick={onClose}
                ...
                <div>
                  ...
                  <select
                    ...
                    className={`w-full p-2 border rounded-md ${
                      fieldErrors.fournisseur_id ? 'border-red-500' : 'border-gray-300'
                    }`}
                  >
                  ...
                  {fieldErrors.fournisseur_id && (
                    <p className="mt-1 text-sm text-red-500">{fieldErrors.fournisseur_id}</p>
                  )}
                </div>
                  ...
                  >
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) => handleProductChange(index, 'prix_achat', e.target.value)}
                        className={`w-full p-2 border rounded-md ${
                          fieldErrors[`produits.${index}.prix_achat`] ? 'border-red-500' : 'border-gray-300'
                        }`}
                      />
                      {fieldErrors[`produits.${index}.prix_achat`] && (
                        <p className="mt-1 text-xs text-red-500">{fieldErrors[`produits.${index}.prix_achat`]}</p>
                      )}
                    </div>
              ...
              {success && successData && (
                <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-md text-green-800">
                  <div className="flex items-center mb-2">
                    ...
                    <h3 className="font-medium">Bon d'entrée mis à jour avec succès!</h3>
                  </div>
                  <p>
                    Le bon d'entrée <span className="font-medium">{successData.data?.reference || formData.reference}</span> a été mis à jour.
                  </p>
                </div>
              )}
            </form>
