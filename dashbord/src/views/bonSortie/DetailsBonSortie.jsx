...
const DetailsBonSortie = () => {
  ...
  const [bonSortie, setBonSortie] = useState(null);
  ...
  useEffect(() => {
    const fetchBonSortie = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://127.0.0.1:8000/api/v1/bonsorties/${id}`);
        if (!response.ok) {
          throw new Error(`Erreur HTTP: ${response.status}`);
        }
        const result = await response.json();
        setBonSortie(result.data);
      } catch (err) {
  ...
  const handlePrint = () => {
    ...
    document.title = `Bon de Sortie - ${bonSortie?.reference || ''}`;
    window.print();
    setTimeout(() => {
  ...
  const { reference, dateSortie, status, prixTotal, user, client, produits } = bonSortie;
    ...
    <div className="bg-gray-50 min-h-screen p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-gray-800">Détails du Bon de Sortie</h1>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{new Date(dateSortie).toLocaleDateString('fr-FR')}</span>
                </div>
                <div className="flex justify-between">
            ...
            <div>
              <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">Client</h2>
              <div className="space-y-2">
                <div className="flex justify-between">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-lg font-semibold mb-4 text-gray-700">Produits sortis</h2>
